...
/*
preprocesses the data to an array usable by the barchart
*/
const preBarData = function(data){
    ...
    // assigns each genre to its respective big or small list
    // depending if the entry-value is lower or higher than the median
    ...
    // assigns each studio to its respective big or small list
    // depending if the entry-value is lower or higher than the median
...
const makeBarGraph = function(data){
    ...
    // adds the radiobuttons
    addBarOptions();
    // updates the bars
    updateBars();
    // adds the options box and the options of the bar graph
    function addBarOptions(){
           // creates svg with options for the barchart
           ...
           // creates the text and radiobuttons for selecting genre or studio
           ...
           bOptions.selectAll("inputBD")
            ...
            // creates the text and radiobuttons for selecting big or small data
            d3.select(".barOptions").append("text")
                                      .text("Select more/less frequent" +
                                            "genres/studios:");
            d3.select(".barOptions").append("br");
            ...
            // creates the text and radiobuttons for selecting amount
            // or percentages
            d3.select(".barOptions").append("text")
...
const updateBars = function(){
    ...
    var dataVar = parseInt(d3.select('input[name="barData"]:checked')
                             .node().value);
    var typeVar = parseInt(d3.select('input[name="barType"]:checked')
                             .node().value);
    var quantVar = parseInt(d3.select('input[name="barQuant"]:checked')
                              .node().value);

    // obtains the selected data
    var data = dataGlob[0][dataVar][typeVar];
    // sorts the data from large to small
    data.sort(function(a, b) {return d3.ascending(a[1], b[1])});

    // isolates the lowest value value from the data
    var min = d3.min(data, function(d){return quant(d[1]);});
    // isolates the highest value value from the data
    var max = d3.max(data, function(d){return quant(d[1]);});

    // defines the domain and ranges of the barscales
    yScaleBar.domain(data.map(function(d){return d[0];}))
    ...
    // gets the absolute largest datapoint from the bigList of genres or studios
    // for consistent colouring
    ...
    rects.enter()
         ...
         .on("mouseover", function(d) {
                          ...
                          // makes the string for the tooltip
         ...
         .on("click", function(d){
                          // updates the heatmap, only if a genre is selected
                          if (dataVar == 0){
                              barSelGenre = d[0];
                              updateHeat();
                          };
